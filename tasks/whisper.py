...
def whisper(input_data: dict):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3', dir=tempfile.gettempdir()) as temp_file:
        temp_file.write(response.content)
        audio_file = open(os.path.join(tempfile.gettempdir(), os.path.basename(temp_file.name)), 'rb')
        oai = OpenAIConnector()
